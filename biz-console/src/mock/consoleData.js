...
export function getEventEndpoints(event, sceneLevel) {
  if (sceneLevel === 'global') {
    return { source: event.globalSource, target: event.globalTarget };
  }
  if (sceneLevel === 'province') {
    return { source: event.provinceSource, target: event.provinceTarget };
  }
  if (sceneLevel === 'city') {
    return { source: event.citySource, target: event.cityTarget };
  }
...
export function getSceneViewState(sceneLevel) {
  if (sceneLevel === 'global') {
    return { longitude: 68, latitude: 26, zoom: 1.85, pitch: 12, bearing: 0 };
  }
  if (sceneLevel === 'province') {
    return { longitude: 108, latitude: 31, zoom: 4.6, pitch: 25, bearing: 0 };
  }
  if (sceneLevel === 'city') {
    return { longitude: 113.8, latitude: 29.8, zoom: 6.8, pitch: 30, bearing: 0 };
  }
