...
export const useConsoleStore = create((set, get) => ({
  ...
  setRequestedDataMode: async (requestedDataMode) => {
    ...
    try {
      ...
      const fallback = await loadConsoleSnapshot('mock');
      set((state) => ({
        requestedDataMode,
        dataStatus: 'fallback',
        dataError: error instanceof Error ? error.message : 'Remote snapshot failed',
  ...
  hydrateConsoleData: async () => {
    ...
    try {
      const initialMode = remoteConfig.defaultMode === 'remote' ? 'remote' : 'mock';
