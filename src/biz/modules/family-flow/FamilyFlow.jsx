...
function getOutreachTone(status) {
  const value = String(status || '');

  if (value.includes('安全') || value.includes('停止')) return 'text-emerald-300';
  if (value.includes('未接通') || value.includes('待联系')) return 'text-amber-300';
  if (value.includes('高风险')) return 'text-rose-300';
...
export default function FamilyFlow() {
  ...
  const familyCases = visibleEvents.map((event) => ({
    ...
    outreachStatus: event.outreachStatus || '',
    outreachNote: event.outreachNote || '',
    outreachUpdatedAt: event.outreachUpdatedAt || '',
    area: event.area,
      ...
      <div className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)_auto] gap-4">
        ...
        <div className="grid min-h-0 gap-3 overflow-auto pr-1">
          {familyCases.map((item) => (
            <div key={item.id} className={`rounded-2xl border p-4 ${item.active ? 'border-sky-400/25 bg-sky-500/10' : 'border-white/5 bg-white/[0.03]'}`}>
              ...
              <p className={`mt-2 text-xs ${getOutreachTone(item.outreachStatus)}`}>确认结果：{item.outreachStatus || '未记录'}</p>
              <p className="mt-1 text-xs text-console-muted">{item.outreachUpdatedAt ? `${item.outreachUpdatedAt} · ${item.outreachNote || '已更新家属侧记录'}` : '家属侧暂未补充外呼记录。'}</p>
        ...
        <div className="rounded-2xl border border-white/5 bg-console-panelSoft p-4 text-sm text-console-muted">
          当前聚焦家庭：{activeEvent ? `${activeEvent.familyCase} / ${activeEvent.familyStatus} / ${activeEvent.outreachStatus || '未记录确认结果'}` : '未选择'}
        </div>
      </div>
    </ModuleCard>
  );
