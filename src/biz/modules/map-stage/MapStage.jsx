...
export default function MapStage() {
  ...
  const layers = useMemo(() => {
    ...
    const pointData = nodes.map((node) => {
      const active = arcData.some((event) => event.active && (event.sourceName === node.name || event.targetName === node.name));
      return { ...node, active };
    });
    ...
    >
      <div className="grid h-full min-h-0 grid-rows-[auto_auto_auto_minmax(0,1fr)] gap-2.5">
        ...
        <div className="relative h-full min-h-0 overflow-hidden rounded-[28px] border border-sky-300/10 bg-[linear-gradient(180deg,#04111f_0%,#07182a_45%,#081424_100%)] shadow-[inset_0_0_0_1px_rgba(56,189,248,0.05),0_0_32px_rgba(14,165,233,0.08)]">
          ...
          <div className="absolute bottom-4 right-4 z-10 rounded-full border border-white/10 bg-slate-950/54 px-3 py-2 backdrop-blur-sm shadow-[0_6px_18px_rgba(2,6,23,0.16)]">
            <div className="flex items-center gap-2">
              <span className="text-[10px] uppercase tracking-[0.16em] text-console-muted">{sceneLevel === 'global' ? '全球' : '省级+'}</span>
              <div className="h-1.5 w-24 rounded-full bg-[linear-gradient(90deg,rgba(59,130,246,0.95)_0%,rgba(250,204,21,0.95)_48%,rgba(244,63,94,0.95)_100%)] shadow-[0_0_12px_rgba(56,189,248,0.14)]" />
